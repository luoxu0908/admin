...
AutoCompleteTextBox.prototype.MatchType = 0;
...
AutoCompleteTextBox.prototype.SetSelection = function () {
	if (this.CurSelection > -1 && this.Div.style.display == 'block') {
		...
		var SelectedValue = '';
		this.TextBox.value = oDiv.innerHTML;
		while (SelectedValue != this.TextBox.value) //remove all <b></b>
		{
			SelectedValue = this.TextBox.value
			this.TextBox.value = this.TextBox.value.replace(/<b>([^<]*)<\/b>/gi, '$1');
		}
...
AutoCompleteTextBox.prototype.Cbo_Complete = function (responseText, responseXML) {
	...
	var i, j, n = aStr.length;

	if (n > 0) {
		var SSS;
		switch (this.MatchType) {
			case 1: //match front
				SSS = new RegExp('^(' + this.TextBox.value.replace(/(\+|\)|\(|\*|\^|\$|\#|\{|\||\[|\\|\?|\.)/gi, '\\$1').replace(/&/gi, '&amp;') + ')', 'gi');
				break;
			case 2: //match anywhere any order
				var matches = this.TextBox.value.replace(/\s+$/gi, '').replace(/^\s+/gi, '').split(' ');
				SSS = new Array(matches.length);
				for (i = 0; i < matches.length; i++) {
					SSS[i] = new RegExp('(' + matches[i].replace(/(\+|\)|\(|\*|\^|\$|\#|\{|\||\[|\\|\?|\.)/gi, '\\$1').replace(/&/gi, '&amp;') + ')', 'gi');
				}
				break;
		...
		for (i = 0; i < n; i++) {
			...
			try {
				switch (this.MatchType) {
					case 2:
						oDiv.innerHTML = aPair[0];
						for (j = 0; j < SSS.length; j++) {
							oDiv.innerHTML = oDiv.innerHTML.replace(SSS[j], '~|$1|~');
						}
						oDiv.innerHTML = oDiv.innerHTML.replace(/\~\|/gi, '<b>').replace(/\|\~/gi, '</b>');
						break;
					default:

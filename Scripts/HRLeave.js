    		...
    		for (var j = 0; j <= NoOfDays; j++) {
    			if (j == 0) {
    				...
    				for (var h = 0; h < window.HolidayList.length; h++) {
    					if (window.HolidayList[h].enddate == null || window.HolidayList[h].enddate == '') { if (date.diff(moment(window.HolidayList[h].startdate), 'days') == 0) { isPH = true; HTMLString += '<td class="'; if (window.HolidayList[h].category === 'PH') { HTMLString += 'ph'; } else { HTMLString += 'block'; } HTMLString += ' ctr">' + moment.weekdaysShort[day] + '<br />' + j + '</td>'; break; } }
    					else { if (date.diff(moment(window.HolidayList[h].startdate), 'days') >= 0 && moment(window.HolidayList[h].enddate).diff(date, 'days') >= 0) { isPH = true; HTMLString += '<td class="'; if (window.HolidayList[h].category === 'PH') { HTMLString += 'ph'; } else { HTMLString += 'block'; } HTMLString += ' ctr">' + moment.weekdaysShort[day] + '<br />' + j + '</td>'; break; } }
    				}
    				if (isPH == false) {
    					if (day == 0 || day == 6) {
    								...
    								if (list.length == 0) { item2.date = seldate; if (R[i].CaseStatus == 'Active') { item2.Code = 'Y'; } else { item2.Code = 'O'; } if (leaveObj.Selection[k].AM == '0') { item2.Duration = 'PM'; } else { item2.Duration = 'AM'; } dates.push(item2); item.Dates = dates; list.push(item); break; }
    									...
    									for (var m = list.length - 1; m < list.length; m++) {
    										if (list[m].PersonID != R[i].PersonID) { item2.date = seldate; if (R[i].CaseStatus == 'Active') { item2.Code = 'Y'; } else { item2.Code = 'O'; } if (leaveObj.Selection[k].AM == '0') { item2.Duration = 'PM'; } else { item2.Duration = 'AM'; } dates.push(item2); item.Dates = dates; list.push(item); break; }
    											...
    											if (exists) {
    												if ((list[m].Dates[index].Code == 'Y' && R[i].CaseStatus != 'Active') || (list[m].Dates[index].Code == 'O' && R[i].CaseStatus == 'Active')) { list[m].Dates[index].Code = 'O'; if (leaveObj.Selection[k].AM === '0') { list[m].Dates[index].Duration = 'PM'; } else { list[m].Dates[index].Duration = 'AM'; } }
    											}
    											else {
        ...
        .then(function (data) {
          ...
          for (var j = 0; j <= NoOfDays; j++) {
            if (j == 0) {
              ...
              HTMLString += '<td class="ctr'; if (day == 0 || day == 6) { HTMLString += ' weekends'; } HTMLString += '"><input type="checkbox" id="cbAM_' + j + '"'; if (WorkingDays.indexOf(day) >= 0 && isPH == false) { if (Balance > 0) { if (Balance > DaysAccummulated || negative == true) { if (day !== 6) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } else { if (Balance.toString().indexOf('.') < 0) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } } } } else { if (lvcat in { 'No Pay Leave': 1, 'National Service Leave': 1 }) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } } } else if (window.CalSelectionConfig == 'No') { HTMLString += ' disabled="disabled"'; } HTMLString += ' /></td>';
              HTMLString += '<td class="ctr'; if (day == 0 || day == 6) { HTMLString += ' weekends'; } HTMLString += '"><input type="checkbox" id="cbPM_' + j + '"'; if (WorkingDays.indexOf(day) >= 0 && isPH == false) { if (Balance > 0) { if (Balance > DaysAccummulated || negative == true) { if (day !== 6) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } else { if (Balance.toString().indexOf('.') < 0) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } } } } else { if (lvcat in { 'No Pay Leave': 1, 'National Service Leave': 1 }) { HTMLString += ' checked="checked"'; DaysAccummulated += 0.5; } } } else if (window.CalSelectionConfig == 'No') { HTMLString += ' disabled="disabled"'; } HTMLString += ' /></td>';
